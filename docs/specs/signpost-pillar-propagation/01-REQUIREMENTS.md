...
- [ ] Given a project's **probe-hook cutover or install** (tier 2/3 — Components 1-3, executable artifacts), when it is called done, then it has passed its own independently dispatched, unbriefed (map-not-route) Frank gate — the sprint-level agent-rig Frank gate does not substitute. **This gate does NOT apply to tier-1 practice-only rollout** (Components 4/5/7): those are prose conventions followed or not in future sessions, not at install time, so a gate can only confirm text presence and tracked status — which the resident agent self-checks. Amended 2026-08-12 (Danny) after an independent Frank gate on the rollout process returned FAIL for scoping a cutover-sized control to a prose-sized change.
...
- [ ] **Sprint forge-phase closability.** The forge phase is closable when Slices 1-9 are complete — the propagation machinery itself, fully within agent-rig's own control. Retrofit rollout to any external project (Slice 10, the `market_data` pilot, together with Slices 11/12 covering `electric-blue`, `gap-lens-dilution`, `gap-lens-dilution-filter`, `ask-edgar-repo`, `sonic-store`, `quant-foundry`, `runtime/agent-lore`) is explicitly not a gate on this sprint's own forge-phase closability — it tracks as a separate, ongoing initiative owned by each project's own resident agent, not blocking this sprint's completion. This is a deliberate decision (2026-08-09): making the retrofit pilot's own PASS a precondition of agent-rig's own gate was rejected as unnecessary ceremony — the retrofit is meant to be generic enough that agent-rig should not force a dependency on another repo's cutover in order to close its own forge session.
...
| Retrofit pilot (Slice 10) and remaining roster items (Slices 11/12) not yet started, or not yet passing, when Slices 1-9 are complete | Sprint's own forge-phase is still closable; Slice 10 and Slices 11/12 track as ongoing, non-blocking, per-project work, not treated as blocking debt on this sprint |
...
- NOT: Full retrofit rollout (Slice 10, the `market_data` pilot, and Slices 11/12, the remaining roster projects) as a condition of this sprint's own forge-phase closability — see Cross-cutting AC on forge-phase closability. Retrofit rollout is a separate, ongoing initiative tracked per-project, not a gate on agent-rig's own sprint close.
...
- Must not: Make the retrofit pilot's (Slice 10) own Frank-gate PASS, or completion of any other retrofit-roster project (Slices 11/12), a precondition of this sprint's own forge-phase closability — agent-rig's own gate closes on Slices 1-9 alone.
