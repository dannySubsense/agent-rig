...
- [ ] Slice 11: Retrofit rollout, practice-only items (remaining roster) — RELEASED 2026-08-12 (Danny: resident agents get full instructions and update their own `CLAUDE.md`). Packet: `SLICE-11-INSTRUCTIONS.md` (SLIMMED 2026-08-12 after Frank FAIL on process proportionality: per-repo Frank gate and blast-radius grep CANCELLED for this slice — a gate cannot verify a habit, and the grep re-derives the roster audit. Gate stays mandatory for Slice 12). Tracked-status is an explicit per-agent step-0 decision, recorded per repo — 6 of 8 roster repos gitignore `CLAUDE.md`, so durability varies by repo and is reported, not assumed (`docs/reports/roster-gitignore-audit-2026-08-12.md`).
