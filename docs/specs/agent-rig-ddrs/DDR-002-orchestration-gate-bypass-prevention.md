...
- **Status:** ACCEPTED — Danny, 2026-07-20, after Frank's re-review PASS on the F1/F2/F3 remediation. §3.5 (Always-Redispatch clause) is implemented and distributed; §3.1–§3.4 (completion-claim keystone, GATE-LOG extension, gated-artifact registry, PreToolUse hook) are committed-to-build as the `orchestration-gate-bypass-prevention` sprint, registry first. Path to acceptance: content review 2026-07-20 → §5 open questions resolved → §3.5 authored → Frank FAIL on distribution (F1/F2) → template moved to agent-rig ownership + `/new-project` repointed → Frank re-review PASS → accepted here.
- **Author:** wright
- **Date:** 2026-07-17 (content review + §5 resolutions + Frank FAIL: 2026-07-20)
- **Sprint (on approval):** §3.1–§3.4 → own sprint, candidate slug `orchestration-gate-bypass-prevention`; §3.5 authored 2026-07-20, distribution pending the F1 location decision
- **Supersedes:** —
- **GitHub issue:** —
...
**Proposed text for Danny's own global `~/.claude/CLAUDE.md`** (the one surface guaranteed to load in every session regardless of project — not agent-rig's to edit unilaterally, since it's Danny's personal file; proposed here for his own placement, same posture as any edit to an identity-tier document). **Placement (resolved 2026-07-20, Danny):** its own top-level section, sibling to "Research Data Integrity" — same category (org-wide binding discipline), different failure mode (completion-claim-as-evidence vs. source/input validity), so it stands parallel rather than nested. **Wording is self-standing (resolved 2026-07-20, Danny):** the rule must not be gated on the registry (§3.3) existing or being current — the registry *enumerates* to sharpen, it does not *authorize*.

> **Gated completion claims.** An artifact that a registered orchestration framework's component skill or subagent could produce — code, a design sheet, a notebook, a spec doc — cannot be reported done, staged, ready, or complete without citing the specific `GATE-LOG.md` entry (path + attempt row) that backs the claim. This binds regardless of which command, skill, or subagent call actually produced the artifact — "I didn't go through the orchestrator" is not an exemption. The rule is **self-standing**: it does not depend on the gated-artifact registry (`agent-rig`'s `docs/GATED-ARTIFACT-REGISTRY.md`) existing or being current. The registry enumerates known gated artifact types to sharpen this rule; an artifact resembling a type a registered framework already covers, produced through an unregistered or unenumerated route, defaults to **escalate, not exempt**. A completion claim without a citable entry is treated as unverified, not as a lower-confidence pass.
...
New tracked file, agent-rig: `docs/GATED-ARTIFACT-REGISTRY.md`. Named owner: Wright. Lists, per orchestration framework, the component skills and subagents whose output is a gated artifact type. **Namespace caution (Frank F3, 2026-07-20):** skills and subagents are *distinct namespaces with different names* — the installed notebook **skills** are `notebook-design`, `notebook-build`, `notebook-execute`, `notebook-qc`, while the **subagents** are `notebook-designer`, `notebook-builder`, `notebook-executor`, `notebook-qc`. The §3.4 hook matches `Skill` tool calls, so the block that feeds it must list **skill** names (e.g. `notebook-start` → skills `notebook-design`, `notebook-build`, `notebook-execute`, `notebook-qc`), not subagent names. Whether the registry also needs a subagent-name column to cover *direct subagent dispatch* (a bypass route the Skill-only hook leaves uncovered — Frank's original Fix #6 residual), and verifying the `forge-start`/`spec-start` entries against the actually-installed skill names the same way, are **spec-time tasks** — the seed above is illustrative and must be reconciled against `~/.claude/skills/` before the registry is written, not copied as-is.
...
## §5 Open Questions — RESOLVED (Danny, 2026-07-20)

- ~~Exact wording/placement mechanics for the §3.1 text in `~/.claude/CLAUDE.md`~~ — **RESOLVED:** own top-level section, sibling to "Research Data Integrity"; rule worded **self-standing** (registry enumerates, does not authorize — see §3.1). Insertion is still Danny's to place in his own personal file; the text is now final-form.
- ~~`docs/GATED-ARTIFACT-REGISTRY.md` shape — markdown vs. structured~~ — **RESOLVED:** markdown (human-canonical, matches every other agent-rig doc), with the gated skill-names in **one greppable block** so there is a single source. The §3.4 hook derives its matcher from that one block rather than hand-maintaining a parallel list — a second list would drift, which is the promoted-default/shared-well failure in miniature. No JSON-parse pipeline: the hook is reminder-only and low-stakes.
- ~~Own sprint vs. folded in~~ — **RESOLVED (split by size):** §3.5 (Always-Redispatch clause) ships now with the 2026-07-20 template refresh — done in the same session, not deferred. §3.1–§3.4 become their own sprint (`orchestration-gate-bypass-prevention`), earning full Intake→spec→forge→Frank rigor rather than being buried in whatever sprint next touches `notebook-start`. **Build order inside that sprint:** registry (§3.3) first, since §3.1's self-standing wording references it as the enumeration.

**Status note (corrected after Frank FAIL, 2026-07-20):** §3.5's clause was *authored* into `/home/d-tuned/agent-rig/HOMELAB-CLAUDE.md.template` (artifact-bound, agent-agnostic per Frank's Fix #2 — the clause text itself passed Frank's re-review). It is **NOT distributed**: `/new-project` reads the template from `~/runtime/agent-lore/HOMELAB-CLAUDE.md.template` (its Substep 4.1, line 366), which is the stale Jun-25 copy without the clause — so no new project gets it. An earlier version of this DDR's status line claimed "§3.5 shipped," which the artifact did not back — the exact completion-claim-as-evidence failure this DDR exists to kill, caught by Frank inside the DDR's own header. Corrected here. Frank's three findings:

- **F1 (blocking):** the two `HOMELAB-CLAUDE.md.template` copies (agent-rig root vs. `~/runtime/agent-lore/`) are out of sync, one canonical master is declared, and it is the stale one `/new-project` reads. Fix requires deciding the canonical location per the 2026-07-17 stewardship transfer and reconciling `/new-project`'s read path — which is coupled to the physical template migration Danny flagged as pending his own confirmation with Cairn. **Owner decision, not Wright's to execute unilaterally.**
- **F2:** the agent-rig template's own Setup section (lines 8–11) still instructs `cp ~/runtime/agent-lore/...` and names agent-lore as master — stewardship moved on paper, the pointer didn't. Fix coupled to F1's location decision.
- **F3:** §3.3's registry seed conflates two distinct namespaces — skill names (`notebook-build`, `notebook-execute`) vs. subagent names (`notebook-builder`, `notebook-executor`) — and the §3.4 hook matches only `Skill` tool calls. Corrected in §3.3 below; the full skill-vs-subagent enumeration (and whether the hook must also cover direct subagent dispatch — Frank's original Fix #6 residual) is flagged there as a spec-time task.

**Remediation (2026-07-20, after Frank FAIL, Danny's directive):**
- **F1 + F2 fixed by moving the template to agent-rig ownership.** Danny decided the physical file moves — source-of-record is now the agent-rig repo, runtime read location is `~/.claude/templates/` (matching every other homelab template), and `/new-project` was repointed off `~/runtime/agent-lore/` (all 8 references; verified 0 residual, global install == tracked copy). `MACHINE-SETUP.md.template` moved alongside per the same directive. The template's own Setup section (F2) was rewritten to name the new canonical path. §3.5's clause now genuinely reaches every new project. The stale `~/runtime/agent-lore/` copies are left orphaned-but-present for Cairn/Danny to retire (coordination ping sent; Danny handles the Cairn conversation next session) — not deleted unilaterally from another agent's runtime dir.
- **F3 fixed** in §3.3 (skill-vs-subagent namespace caution; full enumeration flagged as spec-time).

§3.1–§3.4 still await their own sprint. §3.5 is now distributed.

**Frank re-verification (2026-07-20): PASS.** Frank re-opened every live file himself and confirmed F1/F2/F3 all hold: `/new-project` reads `~/.claude/templates/` with zero residual `runtime/agent-lore` template refs; deployed template carries the clause byte-identical to source; Setup section corrected (the two surviving `agent-lore` refs are the real `.env`/dispatcher paths, correctly not findings); §3.3 skill names match `~/.claude/skills/`. His summary: the fix "eliminated the split-brain entirely rather than patching the pointer — one source-of-record, one deploy target, verified byte-identical, with the stale well left visibly orphaned." Verdict: fit to move to ACCEPTED **on Danny's approval** (still pending — not self-flipped).
