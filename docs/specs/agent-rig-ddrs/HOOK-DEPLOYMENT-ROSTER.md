...
|---|---|---|---|---|---|
| `agent-rig` | Live (`scripts/session_queue_probe.py`) | Live — includes claim-matching (PR #20) + boundary-matching (PR #21) fixes | Yes, on `main` | `no-preamble-no-meta-narration.sh` (DDR-007), `domain-boundary-provenance` (DDR-006) — **built, forge-gate PASSED, PR #11 merged, but NOT wired into `.claude/settings.json`** (no live entry, confirmed by direct read 2026-09-05; extension sprint in progress will wire it under `log_only`), `progress-proof-per-slice` (DDR-008) | 2026-09-05, direct file read (`.claude/settings.json` + `.claude/hooks/`) |
