...
| soundness-language grep test (Slice 9) | 2 | Attempt 1: under-sensitive (missed bare word forms). Attempt 2's `\bvalid\b` fix was itself both too broad (false positive on real "valid JSON"/"valid by construction" wrapper text) and too narrow (`\bvalid\b` doesn't match inside "validated", the case it was meant to catch) — caught by independently re-running the test, not trusting the fix report. Corrected to `\bvalidated\b`. |
