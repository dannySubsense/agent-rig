...
**Status**: DRAFT
**Mode**: spec-lite (per `docs/specs/progress-md-proof-per-slice-hook/INTAKE.md` — no Requirements/
...
`docs/specs/progress-md-proof-per-slice-hook/INTAKE.md` (APPROVED 2026-08-22). **Sibling build,
...
- The literal marker `PROOF:` (case-sensitive, exact string) may appear anywhere in a slice bullet
  ...
  file does), that segment goes *before* `PROOF:`, and `PROOF:` remains present, still last, at the
  point of the completing edit.
...
- **A slice bullet line with no `PROOF:` marker at all, at the moment of its `[ ]`→`[x]` edit, is
  never inspected — allowed unconditionally, same posture as "no manifest" in the sibling hook.**
  Declaring a proof command is opt-in per slice, not mandatory; this hook adds a mechanism, not a
  new requirement that every slice must satisfy to be marked done. (Whether Frank or a human
  reviewer requires proof commands as a matter of process is out of this hook's authority — §9.)
...
5. For each line index `i`, compare `old_lines[i]` and `new_lines[i]` (both stripped of leading
   ...
   `new_lines[i]` starts with `- [x]` AND both refer to the same slice (same text following the
   checkbox, ignoring the checkbox mark itself and any trailing segment appended after it in
   `new_lines[i]` — i.e. `new_lines[i]` must start with `old_lines[i]`'s post-checkbox text as a
   prefix, allowing only appended trailing content, consistent with §4's "append before PROOF:,
   PROOF: stays last" convention). Lines with no transition are skipped.
...
3. A `[ ]`→`[x]` transition on a slice line with no `PROOF:` marker is always allowed
   (opt-in, §4/§6 step 7).
...
11. The self-test fixture corpus exercises, at minimum: out-of-scope filename, no-transition edit,
    ...
    transition denies, per §6 step 10).
