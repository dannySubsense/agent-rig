...
**Status**: LOCKED — approved by Danny 2026-08-24, after Frank's binding spec-gate PASS (attempt 2
of 3, both layers, no PROVISIONAL) and the orchestrator's independent post-PASS review (found and
fixed one additional defect, AC4's non-discriminating example). Ready for implementation handoff
(`/forge-start --lite`).
...
I understand}` (§3.2) is itself a clause boundary — it terminates the matching clause immediately
after the verb phrase, before its complement, even when no punctuation or conjunction separates
them. This exists specifically because these six verbs commonly take a complement clause with no
intervening punctuation ("I think **Frank should review this**," "I believe **the timeout is
wrong**") and, without this boundary, the complement's own concrete nouns would bleed into the
same clause span as the narrating verb and wrongly neutralize it under §3.3 — the complement is a
distinct clause and is evaluated against §3.2/§3.3 independently, on its own text, exactly as any
other clause is (see §3.3's worked example for the resulting behavior). This boundary type applies
only to this six-verb subset, not to the rest of §3.2's verb list (e.g. "I'll," "Let me," "I
should") — those verbs do not reliably take a punctuation-free complement clause in the same way,
and widening the rule further is left to a future amendment (§11) if the log-only soak (§6)
surfaces a comparable case.
...
both under-match (misses `config.yaml`, lowercase) and over-match on a case like "I think Frank
should review this" — under punctuation-only clause boundaries, "Frank" (a capitalized token at
position 2+) would sit in the *same* clause as "I think" and wrongly neutralize it, even though the
clause is pure narration with no substance of its own. §3.1's sixth boundary type (epistemic-verb
identity, added specifically to resolve this case) prevents that: it isolates "I think" as its own
clause, separate from "Frank should review this." "I think" alone then matches §3.2, contains none
of the five concrete signals below, and correctly flags; "Frank should review this" does not begin
with a §3.2 verb sequence and is a distinct clause the rule never touches, matching or not on its
own terms. §3.3's five independent concrete-signal classes, any one sufficient, mirror
defense-in-depth reasoning already used in this repo (`first-turn-contract-enforcement` §4's
layered, not single-point, guarantees) applied to a detection question instead of a safety
question — but they only ever operate within a single clause's own span, never across the §3.1
boundary that keeps a narrating verb and its complement apart.
...
4. A `last_assistant_message` opening with `**HALT:**` or `**BLOCKER:**` followed by first-person
   narration that itself matches §3.2 and contains no §3.3 signal (e.g. `"**HALT:** I need to get
   your decision on which option to take."` — the narrating clause matches §3.2's "I need to" and
   contains no backtick, file-path, capitalized token, digit, or quoted literal) produces `allow` —
   verifies §5.2's structural exemption fires by exercising a clause that would flag without it.
...
- **§3.1's epistemic-verb clause boundary introduces a new, known false-positive class**: a bare
  epistemic-verb clause from the closed six-verb subset (`I think`, `I believe`, `I feel`, `I
  realize`, `I recognize`, `I understand`) now always flags, even when its complement clause carries
  a concrete, substantive claim ("I think the bug is in `parse.py`" flags, because the boundary
  splits the concrete noun `parse.py` into the separate, unevaluated complement clause). This is not
  present in the pre-fix design and is a direct, accepted consequence of resolving Intake Open
  Question 3's "Frank should review this" case in §3.1/§3.3. Forge stage's fixture corpus
  (bullet 1, above) and §6.4's human review of log-only entries must deliberately include and
  correctly classify examples of this shape — a legitimate "I think X is wrong about `Y`" flag
