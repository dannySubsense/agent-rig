...
**Open verification item (owner: wright, before forge):** the exact PreToolUse hook JSON output
schema Claude Code expects for a deny decision (candidate shapes seen in ecosystem documentation:
top-level `{"decision": "block", "reason": ...}` matching the Stop-hook shape this repo already
verified for `first-turn-contract.sh`; or a `hookSpecificOutput.permissionDecision` /
`permissionDecisionReason` shape specific to PreToolUse) has not been live-verified in this
session — no PreToolUse hook exists anywhere in this repo today to confirm against (grep across
`.md`/`.sh`/`.py`/`.json` returned zero PreToolUse deny examples). Per this repo's verification-
class standard (live capture > string-presence-with-context > string-presence-alone), this
architecture does not assert a schema from memory. **Forge must live-verify the actual accepted
deny shape (e.g. via a throwaway hook + Claude Code session capturing the real accepted/rejected
response) before wiring the real hook**, the same way `first-turn-contract-enforcement`'s Intake §0
required for its own field/event claims. Until verified, the wrapper (§6) is written to attempt
the Stop-hook-compatible shape first (since it is the one shape this repo has live-verified
elsewhere) and treat a schema mismatch the same as any other probe failure — fail open (§6).
...
interface DomainBoundaryManifest {
   ...
   * fnmatch/pathlib semantics) identifying files this pipeline treats as its
   * own config/spec surface — the files this hook inspects on Edit/Write.
   * A file not matching any entry is never inspected (silent pass, §5).
   */
...
3. Resolve the file path being written (`tool_input.file_path`). If it matches no glob in
   `pipelineConfigGlobs` → allow, track-record entry `decision: "allow"`, `manifest_status:
   "matched"`, `file_in_scope: false`.
4. Resolve the proposed post-edit content: `tool_input.content` (Write) or the result of applying
   `tool_input.old_string` → `tool_input.new_string` against the current file content, read from
   disk at `tool_input.file_path` (Edit). If the current file cannot be read for an Edit (e.g. does
   not yet exist) → allow, `probe_error: "edit target unreadable"` (fail toward allow, never toward
   a block the probe cannot substantiate).
5. Scan the resolved content for any `externalSourceIdentifiers` match (literal substring or `re:`
   regex, §4). No match → allow, `decision: "allow"`, `matches_found: 0`.
6. For each match, search the 5-line window (§5) for a `DOMAIN-BOUNDARY:` marker line with
   non-empty trailing content. Any match lacking a marker in its window → **deny**.
...
**not itself measured for this probe** (this probe does file I/O — reading the on-disk Edit target
— that the sibling probe does not; the sibling's 167ms/85ms measurements do not transfer). Forge
must measure this probe's actual runtime against representative manifest/file sizes before this
value is treated as settled, per the same standard the sibling wrapper's own comment sets.
 ...
 *  fields this probe uses are listed. */
...
interface DomainBoundaryHookOutput {
  // Deny shape TBD pending forge-time live verification (§3). Written here as the
  // Stop-hook-compatible candidate this repo has already live-verified elsewhere;
  // forge confirms or replaces before wiring.
  decision?: "block";
  reason?: string;
}
...
5. A probe crash, malformed manifest, unreadable Edit target, or wrapper timeout always results in
   allow (fail-open) — verified by a fixture-driven test analogous to
   `tests/test_first_turn_contract_probe.py`'s `probe_error` cases.
...
7. The self-test fixture corpus (§6) exercises, at minimum: no-manifest, out-of-scope file,
   ...
   (boundary case at line 6), and Edit-target-unreadable.
...
- **§3's PreToolUse deny-shape live verification** — blocking for wiring the real hook into any
  repo's `.claude/settings.json`, not blocking for building the probe's decision logic itself.
