...
7. The self-test fixture corpus (§6) exercises, at minimum: no-manifest, out-of-scope file,
   ...
   (boundary case at line 6), and pre-existing-uncited-match-unrelated-edit — an Edit whose
   ...
   scan-surface scope: only `new_string`/`content` is inspected, never the rest of the file).
