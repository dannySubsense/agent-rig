...
`/forge-start --lite` should create `feature/first-turn-contract-c3-path-query-boundary-matching`
and open a draft PR — Git Flow Determination should read this as already-given, not ask again.
Ready for implementation handoff.

**Orchestrator's independent verification**: re-ran §2/§3's worked examples directly (Python,
not just re-reading the walkthrough), plus one case beyond the spec's own examples (`"c3"` vs
`"c3_helper"`, correctly rejected since `_` is a word character) — all match Frank's PASS.
